...
WORKDIR /src
COPY ["ProsperLambda.csproj", "ProsperLambda/"]
RUN dotnet restore "ProsperLambda/ProsperLambda.csproj"

WORKDIR "/src/ProsperLambda"
COPY . .
RUN dotnet build "ProsperLambda.csproj" --configuration Release --output /app/build

FROM build AS publish
RUN dotnet publish "ProsperLambda.csproj" \
...
Execute unit tests
```
cd "ProsperLambda/test/ProsperLambda.Tests"
dotnet test
```

Deploy function to AWS Lambda
```
cd "ProsperLambda/src/ProsperLambda"
